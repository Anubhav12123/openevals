...
class ConcisenessMetric(BaseMetric):
    name = "conciseness"
    description = "Response conciseness relative to prompt length. Ideal: ~3x prompt word count"
    ...
    async def compute(self, request: EvaluationRequest) -> MetricResult:
        ...
        if ratio <= self.IDEAL_RATIO:
            ...
            score = max(0.0, 1.0 - (ratio - self.IDEAL_RATIO) / (self.MAX_RATIO - self.IDEAL_RATIO))
