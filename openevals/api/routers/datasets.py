...
_DATASETS: dict[str, dict] = {
    ...
    "mmlu": {
        ...
        "size": 14000,
        "domain": "multi-domain",
        "description": "MMLU 57-subject benchmark",
    },
}
...
async def upload_dataset(
    dataset: DatasetUpload, api_key: dict = Depends(require_api_key)
):
    if dataset.name in _DATASETS:
        raise HTTPException(
            status_code=409, detail=f"Dataset '{dataset.name}' already exists"
        )
    ...
    return {
        "message": f"Dataset '{dataset.name}' uploaded with {len(dataset.items)} items"
    }
