...
app = typer.Typer(name="openevals", help="OpenEvals — Open-source LLM Evaluation Framework", add_completion=False)
console = Console()
...
def evaluate(
    ...
    response: str = typer.Option(..., "--response", "-r", help="Model response to evaluate"),
    metrics: str = typer.Option(DEFAULT_METRICS, "--metrics", "-m", help="Comma-separated metrics"),
    model: Optional[str] = typer.Option(None, "--model", help="Model name label"),
    context: Optional[str] = typer.Option(None, "--context", "-c", help="Context for RAG metrics"),
    ground_truth: Optional[str] = typer.Option(None, "--ground-truth", "-g", help="Expected answer"),
    ...
    async def _run():
        ...
        return await evaluator.evaluate(
            ...
            context=context, ground_truth=ground_truth, model_name=model,
        )

    with Progress(SpinnerColumn(), TextColumn("[bold blue]Evaluating..."), transient=True) as p:
        p.add_task("eval")
...
@app.command()
def benchmark(
    models: str = typer.Option("gpt-4o", "--models", help="Comma-separated model names"),
    dataset: str = typer.Option("truthfulqa", "--dataset", "-d", help="Dataset: truthfulqa, gsm8k, arc, mmlu"),
    metrics: str = typer.Option(DEFAULT_METRICS, "--metrics", "-m", help="Metrics to evaluate"),
    output: str = typer.Option("benchmark_report.html", "--output", "-o", help="Output HTML report path"),
    samples: int = typer.Option(100, "--samples", "-n", help="Number of dataset samples"),
):
    ...
    console.print("[dim]openevals benchmark --models gpt-4o,claude-3-5-sonnet-20241022 --dataset truthfulqa[/dim]")
...
def leaderboard(
    ...
    data = [
        {"rank": 1, "model": "claude-3-5-sonnet-20241022", "faithfulness": 0.94, "hallucination": 0.96},
        {"rank": 2, "model": "gpt-4o",                    "faithfulness": 0.91, "hallucination": 0.77},
        {"rank": 3, "model": "llama-3-70b",               "faithfulness": 0.85, "hallucination": 0.82},
    ]
    if fmt == "json":
    ...
    for row in data:
        table.add_row(str(row["rank"]), row["model"], f"{row['faithfulness']:.2f}", f"{row['hallucination']:.2f}")
    console.print(table)
